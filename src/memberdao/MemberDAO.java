...
public class MemberDAO {
	...
	public ArrayList<MemberBean> selectMemberList(){
		...
		try{
			...
			System.out.println("getDeatilMember4 에러: " + ex);			
		}finally{
			close(rs);
			close(pstmt);
		}
	...
	public MemberBean selectMember(String id){
		...
		try{
			...
			System.out.println("아오씨:"+rs.getString("MEMBER_EMAIL"));
		}catch(Exception ex){
			System.out.println("getDeatilMember2 에러: " + ex);			
		}finally{
			close(rs);
			close(pstmt);
		}
	...
	public int modifyMember(MemberBean member) {
		...
		int updateCount = 0;
		String sql = "update member1 set member_pw=? , member_name=? , member_age=? , member_gender=? , member_email=?";
		sql+="where member_id=?";
		try {
			pstmt = con.prepareStatement(sql);
